...
with tab_upload:
    ...
    with col1:
        ...
        st.markdown("""
<div class="box-info">
Carica il file Excel o CSV dell'estratto conto bancario dell'impresa per il periodo contestato.
...
""", unsafe_allow_html=True)
        file_banca = st.file_uploader(
            ...
            type=["xlsx", "xls", "csv"],
            key="banca",
            help="Il sistema rileva automaticamente le colonne data, descrizione, entrate e uscite."
        )
    ...
    with col2:
        ...
        st.markdown("""
<div class="box-info">
Carica il registro corrispettivi telematici, il registro IVA vendite o l'elenco fatture attive.
Deve contenere almeno la colonna con l'imponibile/ricavo.
</div>
""", unsafe_allow_html=True)
        file_corr = st.file_uploader(
            ...
            type=["xlsx", "xls", "csv"],
            key="corr",
            help="Il sistema rileva automaticamente le colonne data, imponibile e IVA."
        )
...
⚠️ <strong>Suggerimento:</strong> Se il file non viene letto correttamente, rinominare le colonne chiave 
...
if btn_avvia and file_banca and file_corr:
    ...
    if df_banca is None:
        st.error("❌ Impossibile leggere l'estratto conto. Verifica il formato del file nella scheda 'Formato atteso'.")
        st.stop()

    if df_corr is None:
        st.error("❌ Impossibile leggere il registro corrispettivi. Verifica che il file contenga almeno una colonna 'Imponibile' o 'Ricavo'.")
        st.stop()
